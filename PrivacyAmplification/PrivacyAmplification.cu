...
inline void key2StartRest(unsigned int* recv_key) {
...
void recive() {
    ...
    std::ifstream seedfile(TOEPLITZ_SEED_PATH);
    ...
    {
        std::cout << "File \"" << TOEPLITZ_SEED_PATH << "\" is too short => terminating!" << std::endl;
        exit(1);
        abort();
    }

    char* toeplitz_seed_char = reinterpret_cast<char*>(toeplitz_seed);
    seedfile.read(toeplitz_seed_char, seedfile_length);
    #endif
    ...
    //from the ANU Quantum Random Numbers Server (https://qrng.anu.edu.au/)
    std::ifstream keyfile(KEYFILE_PATH);
    ...
    {
        std::cout << "File \"" << KEYFILE_PATH << "\" is too short => terminating!" << std::endl;
        exit(1);
        abort();
    }
