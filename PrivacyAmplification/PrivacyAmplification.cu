...
void recive() {
    ...
    //VeraCrypt key file (PRF: SHA-512) with quantum_random_numbers_ANU_20Oct2017_100MB_7
    //from the ANU Quantum Random Numbers Server (https://qrng.anu.edu.au/)
    char* filename = "toeplitz_seed.bin";
    std::ifstream infile(filename);

    if (infile.fail())
    {
        std::cout << "Can't open file \"" << filename << "\" => terminating!" << std::endl;
        exit(1);
        abort();
    }
    ...
    if (length < sizeof(toeplitz_seed))
    {
        std::cout << "File \"" << filename << "\" is too short => terminating!" << std::endl;
        exit(1);
        abort();
    }

    char* toeplitz_seed_char = reinterpret_cast<char*>(toeplitz_seed);
    infile.read(toeplitz_seed_char, length);
    #endif
